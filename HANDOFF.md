...
- 위 커밋 확인은 실제 릴리스 이미지, 공개 HTTPS, 외부 coturn과 운영 자격 증명 검증을
  대신하지 않는다.
- 2026-09-05 BATON의 `codex/go-link-integration-20260905` 브랜치 `2ae9875`에 ROUND 방의
  GO 생성·폐기 전달, 인증된 방 조회, 화면의 단축 링크 상태·복사, 운영 조회·재처리와 이전 공개
  도메인 재생을 구현했다. 격리 작업 공간은 `/private/tmp/baton-go-integration-20260905`다.
  원본 `4a9060b`의 팀 권한·내 팀 목록·수정 이력·내 캘린더 구독 목록을 함께 통합했다.
- GO 직접 폐기의 백그라운드 반영, 전달 중지·2분 지연 화면 안내와 5분 전달 대기 경보를 제공한다.
  상태 확인의 최근 시도·정상 확인 시각과 실패 코드를 저장하며, 상태 확인 활성화 중 마지막
  정상 확인 후 기본 15분이 지나면 운영 점검이 실패한다. 실패만으로 저장된 링크를 숨기지 않는다.
  폐기는 생성보다 먼저 처리하고, 전달 한 번의 기본 한도는 20건·다음 요청 시작까지 5초다.
- 전체 빌드·API 계약·실제 서명 JWT와 두 MySQL을 사용하는 GO 연동 검증을 통과했다.
  최신 원본 병합 뒤 관련 DB·권한·계약·두 서비스 검증, 프런트엔드 빌드와 브라우저 49건도 통과했다.
  생성 응답 유실 뒤 도메인 변경·취소·폐기, GO 직접 폐기 뒤 BATON 종료 응답과 URL 제거를 확인했다.
- 원본 반영은 README·API 계약·OpenAPI·생성 타입의 작성 중인 변경과 겹쳐 Git이 중단했다.
  원본 변경은 보존했다. 해당 작업이 커밋되면 최신 원본을 연동 브랜치에 먼저 병합하고 검증한 뒤
  원본에 반영한다. 아직 배포하지 않은 GO DB 변경은 원본의 V34~V37 다음인 V38~V40으로 조정했다.
  이미 운영에 적용한 마이그레이션을 교체하는 절차가 아니다.
- 실제 운영 발급자·HTTPS·JWT 읽기 scope와 설정 활성화, 점검 실패의 알림 경로 연결 및 부하 확인은
  남아 있다. 상태 확인·캡처·전달은 기본 중지하며 기존 방 일괄 생성은 포함하지 않는다.
...
3. BATON 원본에서 작성 중인 변경이 커밋되면 최신 원본을 GO 연동 브랜치에 통합·검증한 뒤
   원본에 반영한다. 실제 서비스 JWT와 HTTPS 환경에서 생성 응답 유실 뒤 취소·폐기 수렴,
   상태 확인 장애 경보와 폐기 우선 묶음 처리를 검증한다.
