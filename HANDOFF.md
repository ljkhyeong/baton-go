...
- 위 커밋 확인은 실제 릴리스 이미지, 공개 HTTPS, 외부 coturn과 운영 자격 증명 검증을
  대신하지 않는다.
- 2026-09-05 BATON의 `codex/go-link-integration-20260905` 브랜치 `cc616b0`에 ROUND 방의
  GO 생성·폐기 전달, 인증된 조회·화면 복사, 운영 조회·재처리와 이전 공개 도메인 재생을 구현했다.
  작업 공간은 `/private/tmp/baton-go-integration-20260905`이며 원본 `21d3de3`까지 통합했다.
...
- 정책·설정, DB·권한·API 계약, 실제 서명 JWT와 두 MySQL을 사용하는 GO 연동 검증을 통과했다.
  최신 원본 병합 뒤 캘린더 구독·계약 검증, 프런트엔드 빌드, 브라우저 42건과 30건도 통과했다.
  기존 운영 사전점검 묶음과 운영 스크립트 26개 정적 분석을 통과했고, 실제 비루트 컨테이너에서
  JWT 읽기·원자 교체 반영·쓰기 차단과 캡처 및 전달 Compose 구성을 확인했다.
- 원본 반영은 작성 중인 계정 비활성화의 방 권한·인증 계약·생성 타입 등과 겹쳐 Git이 중단했다.
  원본 변경은 보존했다. 해당 작업이 커밋되면 최신 원본을 연동 브랜치에 병합하고 관련 권한·DB
  검증 뒤 원본에 반영한다. 미배포 GO DB 변경은 작성 중인 원본 V39 다음의 V40~V42로 정리했다.
