...
- 위 커밋 확인은 실제 릴리스 이미지, 공개 HTTPS, 외부 coturn과 운영 자격 증명 검증을
  대신하지 않는다.
- 2026-09-05 BATON의 `codex/go-link-integration-20260905` 브랜치 `9399fef8`에 ROUND 방의
  GO 생성·폐기 전달, 인증된 조회·화면 복사, 운영 조회·재처리와 이전 공개 도메인 재생을 구현했다.
  작업 공간은 `/private/tmp/baton-go-integration-20260905`이며 원본 `b22d5c2`의 계정 비활성화와
  최신 구독 화면 변경까지 통합했다.
...
- GO 설정은 기존 BATON 운영 환경 검증·사전점검·배포 도구에 연결했다. 캡처만 켜면 JWT가
  필요 없고 전달을 켜면 JWT 디렉터리를 자동 연결한다. 전용 디렉터리 0750·파일 0640과 동일한
...
- 최신 원본 병합 뒤 정책 121건, GO 전달·계정 비활성화·팀 접근 DB 검증, CAL 구독·ROUND 권한
  DB 검증, API 계약과 실제 서명 JWT·두 MySQL을 사용하는 GO 연동 검증을 통과했다.
  프런트엔드 빌드와 PC·모바일·Safari의 계정 비활성화·단축 링크·구독 화면 15건도 통과했다.
  JWT 장애 대응을 포함한 기존 운영 사전점검 묶음과 운영 스크립트 26개 문법·정적 분석도 통과했다.
  앞선 검증에서는 실제 비루트 컨테이너의 JWT 읽기·원자 교체 반영·쓰기 차단을 확인했다.
- 원본 반영은 새 자료 검토 기능에서 작성 중인 `docs/api/openapi3.yaml`과
  `frontend/src/generated/api.ts`에 겹쳐 Git이 중단했다. 원본 변경은 보존했다.
  해당 작업이 커밋되면 최신 원본을 연동 브랜치에 병합하고 관련 API 계약·자료 검토 검증 뒤
  원본에 반영한다. 미배포 GO DB 변경 V40~V42는 통합한 계정 비활성화 V39 다음 순서다.
  운영 DB에 이미 적용한 마이그레이션 파일을 교체하는 절차가 아니다.
- 실제 운영 발급자·HTTPS·JWT 읽기 scope와 설정 활성화, 점검 실패의 알림 경로 연결 및 부하
  확인은 남아 있다. 상태 확인·캡처·전달은 기본 중지하며 기존 방 일괄 생성은 포함하지 않는다.
...
3. BATON 원본에서 작성 중인 자료 검토 변경이 커밋되면 최신 원본을 GO 연동 브랜치에 통합·검증한 뒤
