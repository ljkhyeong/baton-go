...
- 위 커밋 확인은 실제 릴리스 이미지, 공개 HTTPS, 외부 coturn과 운영 자격 증명 검증을
  대신하지 않는다.
- 2026-09-05 BATON의 `codex/go-link-integration-20260905` 브랜치 `2e8ef501`에 ROUND 방의
...
- 운영 도구는 장애 복구 뒤 `CREATE_UNKNOWN`·`REVOKE_PENDING`도 조회한 상태·시도 횟수·오류가
  같고 임대가 없으면 재시도 대기를 앞당긴다. 이미 처리 시각이 되었거나 같은 명령을 반복하면
  변경하지 않는다. 요청 UUID·원격 링크 ID·취소 표식은 보존하고 외부 호출은 전달 작업자가 수행한다.
- BATON 품질 게이트에 고정 GO 소스의 실제 연동 검증 `go_contract`를 추가했다. 검증 커밋은
...
- 원본 반영은 다음 화면 작업에서 작성 중인 `README.md`와 `frontend/src/styles/global.scss`에
  겹쳐 Git이 중단했다. 원본 변경은 보존했다.
  해당 작업이 커밋되면 최신 원본을 연동 브랜치에 병합하고 관련 화면 검증 뒤
...
3. BATON 원본에서 작성 중인 화면 변경이 커밋되면 최신 원본을 GO 연동 브랜치에 통합·검증한 뒤
   원본에 반영한다. GO 검증 소스와 CI 읽기 자격 증명을 준비해 실제 GitHub 품질 게이트를 확인한다.
   실제 서비스 JWT와 HTTPS 환경에서 생성 응답 유실 뒤 취소·폐기 수렴,
