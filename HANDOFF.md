...
- 2026-09-08 추가 문서 정리는 `bc241e2`에서 시작했다. 시작 시 미커밋 변경은 없었다.
  제안한 문구 16곳과 관련 참조를 수정하고 지난 구현·검증 기록을 분리했다.
  문서 내용·로컬 링크 97개(제목 링크 15개 포함)를 확인하고 `git diff --check`를 통과했다.
  명령·설정 예시는 그대로 유지했다. 문서만 변경해 빌드·테스트는 반복하지 않았다.
