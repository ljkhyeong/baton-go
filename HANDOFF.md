...
- 2026-09-08 후속 문서 정리는 `5f2f997`에서 시작했다. 시작 시 미커밋 변경은 없었다.
  README·PRD·ADR·운영 절차의 처리 주체, 동일 요청 재시도와 키 보존 조건을 명확하게 썼다.
  문서 18개의 로컬 링크 70개(제목 링크 13개 포함)를 확인하고 `git diff --check`를 통과했다.
  명령·설정 예시는 유지했다. 결과는 `/private/tmp/baton-go-doc-followup-check-20260908.log`에 있다.
