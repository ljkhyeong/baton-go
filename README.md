...
외부 API·웹훅의 환경변수 예시와 운영 계정 없는 검증은
[코드·연동 준비](docs/RUNBOOK/external-api-integrations.md#코드연동-준비)를 따른다.
...
- 활성 전 링크의 이용 시작 시각을 한국 시간으로 안내
- 활성 전·요청 제한·일시적 서버 오류 화면에서 같은 링크를 다시 여는 버튼
- 발급자 서명 JWT와 작업별 scope로 보호하는 `/api/v1/links` 생성·조회·폐기 API
- 관리 조회·폐기 응답의 링크 상태와 서버 판정 시각
- 대상 시스템·생성 기간·만료 기간·링크 상태 필터와 커서 방식의 관리 링크 목록 조회
