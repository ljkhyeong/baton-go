...
- `POST /api/v1/links`는 링크 생성 요청마다 소문자 정규 UUID `Idempotency-Key`를 요구한다.
  ...
  않는다.
- 과거 배포에서 이미 성공한 링크 생성 요청의 재생 보장을 지키기 위해 당시 UUID 파서가 허용한
...
- 과거 배포가 지원 저장 범위 안의 더 세밀한 입력을 마이크로초로 절삭해 이미 저장한
  링크 생성 요청만 재생 전용으로 호환한다. 기존 예약을 먼저 조회하고 같은 마이크로초 절삭 요청 내용이
...
기존 예약을 읽은 뒤 원시 재생 조회 결과, 정규 대상, 마이크로초 요청 내용과
