...
필요하다. 토큰 파싱, 상수 시간 비교와 설정 검증도 BATON GO가 직접 구현했다.
...
- `BATON_GO_MANAGEMENT_JWT_ISSUER_URI`는 신뢰할 발급자 식별자로 필수다. Spring Boot 표준
  ...
  명시해 애플리케이션 시작 시 발급자 메타데이터 조회를 생략한다. 애플리케이션은 두
  주소의 운영 HTTP 설정을 시작 단계에서 거부하며 루프백 HTTP는 로컬 개발에서만
  허용한다.
...
요청에 `500 INTERNAL_ERROR`를 반환한다. 단축 링크 접속 처리는 이 의존성의 영향을 받지 않는다.

## 결과

- 호출자와 작업 권한을 분리하고 발급자 감사 기록과 키 회전을 사용할 수 있다.
- 직접 구현한 Bearer 파서, 공유 토큰 비교, 관리 토큰 설정과 HMAC 비밀 분리 검사가 제거된다.
- 관리 API에서 발급자 JWK HTTPS 접근과 캐시·키 교체 검증을 운영해야 한다.
