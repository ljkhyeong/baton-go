...
Redis Lua 한 번으로 카운터 읽기·허용·증가·TTL 설정을 원자적으로 수행한다. 첫 요청부터
고정 길이의 창을 사용하므로 창 경계 주변에서는 두 창의 허용량이 연속 도착할 수 있다.
거절한 요청은 카운터와 만료를 연장하지 않는다. 재시도 대기 시간은 Redis의 남은 TTL을
초 단위로 올림하여 기존 `429`의 `Retry-After`에 넣는다. 서버별 시계 차이는 창 계산에 쓰지 않는다.
...
같은 Redis DB·용량·창 설정으로 활성화한다. Redis DB는 환경별로 분리한다. Redis key의 조기
퇴거나 failover 시 미복제 쓰기 손실은 일시적으로 허용량을 늘릴 수 있으므로 Redis는
`noeviction`과 운영 고가용성 정책을 사용하고 기존 로컬 한도도 유지한다. 절대적인 과금·사용권
