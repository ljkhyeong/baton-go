# ADR-0013: 공개 링크의 Redis 분산 요청 제한
...
기존 인스턴스별 요청 제한을 통과한 공개 `GET·HEAD /l/{code}`만 Redis를 확인하고, 한도를 통과한
요청만 MySQL 링크 조회로 진행한다. 관리 API와 다른 경로는 영향을 받지 않는다.

개별 링크 코드·IP·전달 헤더를 키로 사용하지 않는다. 분산 제한은 전체 DB 유입량을 줄이며
클라이언트별 공정한 배분이나 네트워크 DDoS 방어를 제공하지 않는다. 클라이언트별 제한과
DDoS 방어는 공개 외부 프록시에서 적용한다. 공개 접근 로그의 코드 가림도 외부 프록시가 담당한다.
