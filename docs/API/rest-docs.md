...
다음 명령은 링크 생성·재생·관리 조회·폐기, 공개 `GET·HEAD`와 한글 오류 화면, 관리 인증 실패와
...
CI의 `필수 검증` 작업은 같은 파일을 `baton-go-rest-docs` 산출물로 보존한다. 보존 기간은
...
- `links-resolve`
- `links-resolve-head`
- `links-unavailable-html-not_active`
- `links-unavailable-html-expired`
- `links-unavailable-html-revoked`
