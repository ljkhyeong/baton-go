...
- 관리 조회·폐기는 단축 링크 접속 처리와 같은 시간·폐기 정책의 링크 상태와 판정 시각을 반환한다.
  생성·재시도 응답과 저장 스키마는 유지하며 BATON·ROUND 접근 권한을 나타내지 않는다.
- 일반 관리 목록은 기존 읽기 scope로 대상 시스템·생성 기간·링크 상태를 검색한다.
  조회량을 제한하고 필터로 결과가 비어도 다음 커서로 진행하며, 허용되지 않은 저장 대상은 제외한다.
...
- 링크 코드 HMAC 키 등록 검사, 재시도 시 최초 공개 출처 유지와 MySQL 절대 시각 저장 결정은
...
- 2026-09-07 같은 BATON 브랜치 `2852bd95`에 카카오톡 공유와 QR 표시·PNG 저장을 추가했다.
  ...
  사용을 끄는 절차를 문서화했다. 키가 등록되지 않은 상태의 실제 메시지 전송·무료 사용 설정·실기기 스캔은 검증하지 않았다.
...
- 최신 BATON `main`과 GO 연동 브랜치의 병합은 남아 있다. 2026-09-07 원본 BATON 작업 공간은
  ...
  아직 배포하지 않은 GO DB 변경 V40~V42는 통합한 계정 비활성화 V39 다음 순서다.
