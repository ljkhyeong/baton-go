...
관리 API의 쿼리·경로나 JSON 본문 값 형식이 잘못되면 `expiresAt: 요청 값이 올바르지 않습니다`처럼
문제 항목을 안내한다. 필수 쿼리가 없으면 `linkIds: 필수 요청 값이 없습니다`로 안내한다.
문제 항목을 알 수 없는 JSON 구문 오류와 등록되지 않은 항목은 `요청 형식이 올바르지 않습니다`로 안내한다.
...
- `targetSystem`과 `purpose`는 계약에 정의된 대문자 열거형 이름의 JSON 문자열만 허용한다.
  앞뒤 공백은 제거하며, 공백만 다른 재시도는 같은 요청으로 처리한다. 숫자 토큰, 숫자 문자열,
  소문자와 알 수 없는 값은 링크나 생성 예약을 저장하지 않고 `400 INVALID_REQUEST`로 거부한다.
- 시간 초과나 일시적인 `5xx` 뒤에는 동일한 키와 요청 내용으로 재시도할 수 있다.
- 생성 예약에는 발급에 사용한 키 ID를 저장한다. 기존 예약은 `legacy`에 대응하며 현재 발급
  키를 교체해도 기존 요청에는 예약에 저장된 키를 사용한다. 키 ID와 지문은 응답에 포함하지 않는다.
- 재시도 시 저장된 키를 사용할 수 없거나 해당 키의 코드 파생 결과가 저장 코드 해시와 다르면
...
`RESOURCE_OPEN`과 표 밖 조합, 형식에 맞지 않는 식별자, 끝 슬래시와 추가 경로 구간은
`400 INVALID_LINK`로 거부하며 링크와 생성 예약을 저장하지 않는다. 세부 경로 형식, 클릭
...
## GET `/api/v1/links/batch`

이미 알고 있는 여러 링크의 상태를 한 요청으로 확인한다. `baton-go.links.read` scope가 필요하며,
`HEAD`도 같은 권한과 조회 조건을 사용한다.

- 필수 쿼리 매개변수 `linkIds`에 UUID를 쉼표로 구분해 지정한다. 예:
  `?linkIds=83a430c4-5c5d-4eb4-a815-7a5ba1fd4aae,00000000-0000-4000-8000-000000000001`
- 입력 개수는 중복을 포함해 `1..100`이다. 빈 값·누락된 항목·잘못된 UUID·한도 초과는
  `400 INVALID_REQUEST`로 거부한다. 같은 ID를 여러 번 보내면 첫 번째 위치만 유지한다.
- 성공 시 `200 OK`와 `items`, `notFoundIds`, `evaluatedAt`을 반환한다. `items`는 단건 관리
  조회와 같은 형식이며 요청 순서를 유지한다. 공개 코드·코드 해시·멱등성 키·`shortUrl`은 반환하지 않는다.
- 없거나 자동 정리된 링크, 대상 계약을 위반한 링크는 `notFoundIds`에 요청 순서대로 담는다.
  이 사유들은 구분하지 않으며, 모든 ID를 찾지 못한 경우에도 `200`과 빈 `items`를 반환한다.
- 한 번의 기본 키 조회로 요청한 링크만 읽는다. 커서와 검색 필터를 적용하지 않으며,
  결과 전체와 각 항목의 상태를 DB 조회 후 얻은 하나의 `evaluatedAt`으로 판정한다.
- 조회는 링크 상태를 바꾸지 않는다. 이후 이용 가능성이나 BATON·ROUND 접근 권한을 보장하지 않으므로,
  연동 서비스는 백그라운드 상태 확인 등에 사용하고 일반 화면 조회 트랜잭션과 분리한다.

```json
{
  "items": [],
  "notFoundIds": ["00000000-0000-4000-8000-000000000001"],
  "evaluatedAt": "2026-07-29T12:00:00Z"
}
```
...
`status`는 DB 조회 후 서버의 `Clock`으로 얻은 `evaluatedAt` 시점의 GO 링크 상태다.
...
링크별 GO 동기 호출을 추가하지 않는다. 생성·동일 요청 재시도 응답에는 두 필드를 추가하지 않는다.
...
`HEAD`에는 본문이 없다. 내부 지표와 로그로 운영 경보를 남기되 공개 코드,
대상 경로와 전체 단축 URL은 기록하지 않는다.

### 브라우저 오류 안내

- `Accept` 헤더에서 HTML을 우선하는 브라우저에는 없는 링크와 활성 전·만료·폐기 링크, GO가 처리한
...
- 활성 전 링크는 보낸 사람에게 이용 시간을 확인하도록, 만료·폐기 링크는 새 링크를
...
- 활성 전 화면은 저장된 `notBefore`를 `Asia/Seoul`로 변환해
  `2026-07-30 00:30:00 (한국 시간)부터 이용할 수 있습니다.`처럼 안내한다.
  소수초가 있으면 정밀도를 유지하며, 시작 시각을 알 수 없으면 기존 확인 안내를 표시한다.
  이 시각은 HTML에만 표시하고 JSON 오류 필드·메시지와 실제 활성 판정은 유지한다.
- 활성 전·`429`·`500`·`503` 화면은 같은 주소를 다시 요청하는 `다시 열기` 버튼을 제공한다.
  사용자가 누를 때만 요청하고 링크 상태를 다시 판정한다. `429`에서는 안내된 대기 시간 뒤에 사용한다.
  만료·폐기·없는 링크와 저장 대상 계약 위반 화면에는 이 버튼을 표시하지 않는다.
  버튼은 빈 `href`의 기본 링크를 사용해 공개 코드나 현재 URL을 HTML에 삽입하지 않는다.
- 저장 대상 계약 위반은 없는 링크와 같은 화면을 반환하고 기존 내부 지표·로그만 남긴다.
...
용량과 시간 구간은 배포 환경이 명시적으로 설정하는 전체 요청 한도이며 사용자별 제품 할당량이
아니다. 한도를 초과하면 `429 RATE_LIMIT_EXCEEDED`와 현재 시간 구간이 갱신될 때까지의
초 단위 대기 시간을 담은 `Retry-After`를 반환한다.

`HEAD`의 `429`도 같은 상태와 헤더를 반환하지만 응답 본문은 없다. 인스턴스별 제한은
