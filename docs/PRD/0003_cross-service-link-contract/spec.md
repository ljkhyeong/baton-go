...
| 생성 호출자 | 원본 애그리게이트, 링크 생성 요청 UUID, 재시도·보상·폐기 요청 | GO 내부 링크 코드와 대상 서비스의 최종 권한 |
...
- 호출자는 생성·폐기 요청과 같은 `Idempotency-Key`를 소유 트랜잭션의 순서 보장 아웃박스에
  저장한다. 생성 응답이 유실된 뒤 취소해도 같은 요청으로 생성 결과를 확인하고 폐기를 완료하는지 검증한다.
