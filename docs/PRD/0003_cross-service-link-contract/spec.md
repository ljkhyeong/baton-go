...
BATON `RoleResource`의 임의 URL 필드는 이 권위 있는 매핑이 아니다. 방
...
- BATON 계정 세션·CSRF, 신규 브라우저 권한 수령, 일대일 활성 방 매핑과 영구 종료 표식,
  참여권·JWK·쿠키 계약을 실제 공개 경로에서 검증한다.
- BATON과 ROUND의 동일 HTTPS 출처, 정확한 `Origin`, CORS 미사용, 쿠키·헤더 보존,
  경로 우선순위와 사전 인증 요청 한도를 경계 E2E로 검증한다.
- 호출자는 생성·폐기 의도와 같은 `Idempotency-Key`를 소유 트랜잭션의 순서 보장 아웃박스에
  저장하고, 응답 유실·재시도·취소가 생성 후 폐기로 수렴함을 검증한다.
