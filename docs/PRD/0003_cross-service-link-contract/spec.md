...
| BATON | 팀·시즌·리소스와 방 매핑, 방 삭제 표식, 작업 공간 접근 키, 계정 세션·멤버십, ROUND 참여권·JWK 발급 | GO 공개 코드와 링크 수명주기, ROUND 시그널링 입장 |
| ROUND | 정규 방 ID, 입장 전 확인, 참여권 검증, 피어·시그널링·TURN 입장 | BATON 사용자·멤버십, GO 링크 상태 |
| 생성 호출자 | 원본 데이터, 링크 생성 요청 UUID, 재시도·보상·폐기 요청 | GO 내부 링크 코드와 대상 서비스의 최종 권한 |

위치 식별자의 ID는 위치를 찾기 위한 값일 뿐 권한 증명이 아니다. BATON 접근 키, 관리 JWT,
사용자 세션 식별자, CSRF 토큰, ROUND 참여 허가와 TURN
...
- 생산자는 앞뒤 공백, 끝 슬래시, 대문자 UUID·방 ID 또는 정해진 형식이 아닌 구분자를
...
- 알려지지 않은 열거형 문자열처럼 JSON 자체가 역직렬화되지 않는 입력은 PRD-0002에 따라
  `400 INVALID_REQUEST`다. `INVALID_LINK`는 알려진 값으로 만든 허용되지 않은 조합과 위치 식별자에 쓴다.
...
BATON v1 위치 식별자는 실제 화면 경로인 `/teams/{teamId}/seasons/{seasonId}`다. BATON의
...
BATON 공개 출처로 설정한다. `targetSystem`은 경로를 처리하는 서비스를 구분할 뿐 별도
...
수 있는 표준 형식이 아닌 IPv4 주소는 루프백으로 간주하지 않는다.
...
참여 허가 갱신 경로는 범용 `/round/**` 프록시보다 우선한다. `/room/**`가 BATON SPA로,
...
- 세션이 없으면 `200 OK`, `Cache-Control: no-store`와
  `{"authenticated":false}`만 반환한다. CSRF와 계정 필드는 포함하지 않는다.

이 `GET`은 세션을 새로 발급하거나 작업 공간 권한을 부여하지 않는다.

### 5.3 자료–방 연결 정보와 참여권 갱신
...
3. BATON은 현재 계정의 팀·시즌 회의 참여 권한과 BATON DB의 자료–방 연결 정보를
...
GO 링크 생성 전에 커밋하며, 종료 즉시 참여 허가 갱신을 거부한다.
GO 링크는 별도 작업에서 폐기가 완료될 때까지 재시도한다.

BATON `RoleResource`의 임의 URL 필드를 이 연결 정보로 사용하지 않는다.
방 연결을 관리하는 별도 모델이나 동등한 DB 제약을 추가해 활성 `roomId`와 활성 `resourceId`
양쪽의 유일성을 트랜잭션 안에서 보장해야 한다.

BATON UI가 같은 출처의 세션 저장소에 다음 방 연결 정보를 둘 수 있다.
...
키는 `baton-round-entry:v1:{roomId}`다. 이 값은 방 연결을 확인할 때 참고하며 자격 증명이나
권한 증명으로 사용하지 않는다. BATON은 값이 있으면 DB의 자료–방 연결 정보와 정확히 일치하는지 다시
...
BATON은 공개 HTTPS 출처와 일치하는 `Origin`, `Sec-Fetch-Site: same-origin`, 유효한 세션과 동적 CSRF
...
브라우저는 응답 수신 후 `refreshAfterSeconds`가 지나면 갱신한다.
시스템 시각 변경의 영향을 받지 않는 경과 시간을 사용한다.
...
| `study_id` | BATON DB의 자료–방 연결 정보에 저장된 `teamId` 정규 UUID |
| `jti` | 갱신마다 새로 만든 정규 UUID |
| `room_id` | 경로와 매핑의 정규 ROUND `roomId` |
| `role` | v1 고정 `participant` |

v1은 `host`를 추측해 발급하지 않는다. 명시적인 회의 주최자 권한과 매핑 규칙을 도입하는
새 계약 버전 전까지 `host`는 예약값이다.
...
한다. v1 참여 허가는 **일회성 티켓이 아니다**. 동일 `jti`의 동시
...
정상 재연결 중첩용이며 초과 요청은 기존 연결을 끊지 않고 `429`로 거부한다.
소켓의 만료 시각은 연결 시 사용한 참여 허가의 `exp`로 고정한다.
...
- ROUND `ALLOWED_ORIGINS`에는 BATON의 공개 HTTPS 출처만 둔다. 와일드카드, `null`,
  루프백이 아닌 HTTP와 CORS 대체 동작은 허용하지 않는다.
...
- 시그널링은 `ROUND_AUTH_MAX_GRANT_LIFETIME_SECONDS=300`, 계약에 명시한 발급자,
...
| 결과 | 호출자 상태와 처리 |
| --- | --- |
| 최초 생성 `201`·동일 요청 재시도 `200` | `ACTIVE`; 취소 삭제 표식이 있으면 공개하지 않고 즉시 폐기 |
...
| `LINK_CODE_REPLAY_UNAVAILABLE`·`LINK_CODE_CONFIGURATION_MISMATCH` | `RECOVERY_REQUIRED`; 설정 복구 뒤 같은 키로 존재 여부와 `linkId`를 확인 |

한 번이라도 `CREATE_UNKNOWN`이 된 요청에는 뒤의 `400`·`401`만으로 `NO_LINK`를 선언하지
않는다. 자격 증명·배포 차이를 복구한 뒤 같은 요청을 재시도하거나 운영 점검으로 링크가 없음을 확인한다.

원본 리소스가 삭제·종료·권한 회수되면 호출자는 아웃박스나 동등한 재시도 가능한 방식으로
GO 폐기를 요청한다. 폐기가 지연되는 동안에도 BATON 또는 ROUND의 최종 권한 검사가 접근을
막아야 한다. 대상 변경은 기존 링크를 수정하지 않고 새 요청으로 생성한 뒤 이전 링크를
폐기한다.

생성 결과를 받기 전에 원본이 취소되면 호출자는 요청에 `CANCEL_REQUESTED` 삭제 표식을
남긴다. `CREATE_UNKNOWN`은 같은 키로 생성 요청을 재시도해 `linkId`를 확인한 즉시
폐기를 요청한다. 단축 URL은 노출하지 않는다. 확정적 `CREATE_REJECTED/NO_LINK`는 폐기 없이 취소를
완료한다. `CREATE_CONFLICT`와 `RECOVERY_REQUIRED`는 운영자가 원래 요청 내용과 존재 여부를
확정할 때까지 취소를 완료하지 않는다. 생성과 폐기 아웃박스는 같은 링크 생성 요청의 처리 순서를 보장한다.

## 7. 실패와 보상 의미

| 상황 | 계약된 결과 |
| --- | --- |
| 원본 커밋 뒤 GO 생성 실패 | 원본 트랜잭션을 롤백하지 않고 같은 요청을 재시도하며 링크 상태를 대기로 유지 |
| GO가 저장했지만 응답 유실 | 동일 키와 요청 내용으로 재시도해 기존 링크 확인 |
| 생성 대기 중 원본 취소 | `CREATE_UNKNOWN`이면 `linkId` 확인 뒤 폐기, 확정적 `NO_LINK`면 즉시 완료, 충돌·복구 상태면 운영 확인 |
| GO의 링크 접속 처리 장애 | 링크 열기만 실패하며 BATON·ROUND의 직접 조회·편집은 계속 가능 |
| BATON·ROUND 장애 | GO는 대체 출처나 다른 대상으로 보내지 않음 |
| 원본 권한 회수 뒤 GO 폐기 지연 | 대상 서비스가 즉시 접근을 거부하고 별도 작업에서 GO 폐기 완료까지 재시도 |
| 존재하지 않는 팀·시즌·방 | 대상 서비스가 자체 정책에 따라 찾을 수 없음 또는 접근 거부 응답을 반환 |
| 봇·메신저의 선조회 | 도착 화면만 열 수 있고 권한·참여 허가를 소비하거나 발급하지 않음 |

GO는 링크 접속 시 BATON 또는 ROUND를 동기 조회하지 않는다.
대상 서비스도 일반 조회 API에서 링크별로 GO를 호출하지 않는다.

## 8. 호환성과 기존 링크 정책

- `BATON`, `ROUND`, `NAVIGATION`, `MEETING_ENTRY`, `RESOURCE_OPEN` 문자열은 전송·DB 값으로
  동결한다. 이름 변경·삭제는 마이그레이션을 포함한 별도 호환성 파괴 변경이다.
- v1 경로를 제공하는 서비스는 아직 유효한 GO 링크가 있는 동안 정규 경로를 유지하거나, 새 링크
  발급과 이전 링크 폐기를 함께 배포한다.
- 계약 강화 전에 생성된 링크를 자동으로 기존 예외로 인정하지 않는다. 배포 전에 점검해
  표의 정규 조합과 일치하는 링크만 유지하고 나머지는 폐기·재발급한다.
- GO는 링크 접속 처리 중 DB의 계약 위반 대상을 발견하면 존재 여부를 드러내지 않도록
  `404 LINK_NOT_FOUND`로 응답하고 `Location`을 반환하지 않는다. 수동 DB 적재나 잘못된
  마이그레이션도 우회 경로가 되어서는 안 된다.
- GO는 저장된 열거형을 원문 문자열로 읽은 뒤 같은 대상 규칙을 적용한다. 따라서
  알려진 값의 허용되지 않은 조합뿐 아니라 알 수 없는 열거형도 `GET`과 `HEAD`에서 같은
  `404 LINK_NOT_FOUND`로 숨기고 리다이렉트하지 않는다.
- 저장 대상 계약 위반마다
  `baton.go.public.resolver.target.contract.violations` 지표를 증가시키고 `linkId`와 `requestId`만
  포함한 로그를 남긴다. 공개 코드, 대상 경로와 전체 단축 URL은 지표 태그나 로그에
  넣지 않는다.
...
- 배포 DB의 모든 링크를 점검하고 허용되지 않은 링크를 폐기·재발급한 뒤
  `unrevoked non-compliant=0`, 승인되지 않은 `HOLD=0`을 확인한다.
- BATON 계정 세션·CSRF, 접근 키가 없는 신규 브라우저의 기존 BATON 공유 링크 안내,
  일대일 활성 방 매핑과 영구 종료 표식, 참여권·JWK·쿠키 계약을 외부 공개 주소에서 검증한다.
- BATON과 ROUND의 동일 HTTPS 출처, 출처와 일치하는 `Origin`, CORS 미사용, 쿠키·헤더 유지,
...
- 외부 프록시에서 공개 `/l`과 비공개 `/api/v1` 라우팅을 분리하고 분산 요청 제한과 접근 로그 코드 마스킹을
  적용한다.
- 실제 클러스터에서 DNS·TLS·CNI·NetworkPolicy·probe와 PVC·Secret의 생성·교체·복구·삭제를
  검증하고 플랫폼 백업 정책에 따른 격리 복원 훈련을 완료한다.
- ROUND가 외부 상태 저장을 도입하기 전에는 시그널링을 단일 복제본으로 운영한다.

운영 API와 이 절차가 있어도 운영 데이터 폐기나 공개 운영 승인이 끝난 것은 아니다.

## 10. 계약 검증 시나리오

- 계약에 정의한 두 경로만 생성되고 재시도 시 요청 내용 비교에도 같은 값이 사용된다.
- UUID 대문자, ROUND 금지 문자, 끝 슬래시, 추가 경로 구간과 표 밖 목적 조합은
  `400 INVALID_LINK`이며 DB에 쓰이지 않는다.
- 이전 형식 또는 수동 적재된 허용되지 않은 대상은 해석에서 `404 LINK_NOT_FOUND`이며
  리다이렉트되지 않는다.
...
- 세션·CSRF 조건이나 갱신 요청·응답 형식이 계약과 다르면 요청을 거부한다.
- 동일 방의 두 활성 매핑, 삭제 표식이 있는 방 재사용, `study_id != teamId` 또는 v1 `host`
  클레임 발급은 거부한다.
- 참여 허가 갱신 실패, 쿠키/JWT 만료 불일치, JWK에 게시되지 않은 `kid` 또는 방 클레임 불일치 때 TURN과
  시그널링 연결이 시작되지 않는다.
- 300초 이외 참여 허가 수명, 240초 이외 갱신 지연 또는 60초 미만 TURN 잔여 수명은
  배포·계약 테스트에서 거부한다.
- JWK 키 교체는 새 키 선게시와 이전 키의 360초 이상 중첩을 검증한다.
- 원격 생성 시간 초과 뒤 같은 요청을 재시도해도 링크는 하나만 만든다. 원본 권한이 회수되면
  대상 서비스가 접근을 거부하고 GO 링크를 폐기한다.
- 생성 결과를 모르는 상태에서 취소해도 `linkId` 확인 뒤 폐기되어 활성 고아 링크가
  남지 않는다.
