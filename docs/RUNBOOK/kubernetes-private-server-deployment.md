...
9. 관리 JWT 발급자, `aud=baton-go`, 서비스 신원별 scope와 signing key 회전 담당자를
   확정한다. 발급 토큰에 공백이 아닌 안정적인 `sub`와 유효한 `exp`가 있는지 확인하고,
   `sub` 누락·빈 문자열·공백 문자열과 `exp` 누락·만료 요청이 `401`로 거부되는지 검증한다.
   애플리케이션 Pod에서 설정한 JWK Set HTTPS 주소에 접근할 수 있어야 한다.
...
GHCR을 사용하면 [이미지 게시 절차](image-security-reports.md#ghcr-자동-게시와-배포-참조)에 따라
`ghcr.io/ljkhyeong/baton-go`와 해당 실행의 게시 다이제스트를 사용한다.
...
렌더링 결과에는 Secret 객체나 값, 인증서와 신뢰 저장소가 들어가지 않는다. `base`는 외부
...
audience·서비스 식별자·만료 검증과 JWK 캐시는 기존 Spring Security 동작을 유지한다.
...
Pod를 재시작하면 초기화 스크립트도 다시 실행된다고 가정하지 않고 아래 부분 초기화 절차를 따른다.
...
PVC에는 사용자 분리 초기화 스크립트도 다시 실행되지 않는다. 기존 PVC에 런타임·마이그레이션
사용자 분리를 도입하려면 먼저
...
Job이 `Complete`가 될 때까지 유지한다. 완료된 Job을 삭제하고 새로 만드는 경우에도
마이그레이션 완료 전에는 마지막 두 애플리케이션 규모 조정·배포 명령을 실행하지 않는다.
...
- `baton_go_migrator` 계정과 Flyway 스키마 이력을 확인하고 계정의 실제 비밀번호를 먼저
  복구한 뒤 같은 비밀번호가 든 Secret 버전을 맞춘다. Secret만 먼저 바꾸지 않는다.
- 로컬 root 계정 접근을 복구할 수 없거나 시스템 스키마 손상 가능성이 있으면 임의 초기화 스크립트
  재실행이나 데이터 디렉터리 파일 삭제를 하지 않고 DBA와 백업·복원 담당자에게 즉시 보고한다.
- 계정 수동 복구 뒤 마이그레이션 Job `Complete`, 런타임 사용자의 DML 성공·DDL 거부, 애플리케이션
  준비 상태를 차례로 확인한다.

## 5. 접근 제한

`base` 매니페스트는 `Service/baton-go-http:8080`만 만든다. 이 포트에는 다음 경로가 함께 있다.
...
`base`에 예시 selector를 고정하지 않는다. DNS 동작 확인 없이 egress 기본 차단만 적용해
시작·준비 탐지를 깨뜨리지 않는다.
...
- Job이 없다는 이유만으로 성공으로 판단하지 않는다. 직전
...
`Complete` Job을 위 명령으로 삭제했거나, Job 없음 분기에서 직전 완료 기록과 현재 Flyway
...
- 관리 JWT signing key: 발급자가 새 공개키를 JWK Set에 먼저 게시하고 새 `kid`로 JWT 발급을
  ...
  장애 응답의 공통 오류 본문·`requestId`와 로그를 확인하고, 발급자의 가용성 및 Pod의
  JWK HTTPS 접근을 점검한다. 공개 `/l` 링크 접속은 계속 처리되는지 함께 확인한다.
...
- HMAC 비밀값: 기존 키 ID의 값을 바꾸지 않고 [키 교체 절차](link-code-key-rotation.md)에 따라
  ...
  해당 백업에서 기존 URL을 복원하는 데 필요한 키 묶음을 함께 사용한다. 유출·오용이
...
버전을 주입하고 키 등록 검사와 준비 상태를 확인한다. 보관한 검증용 생성 요청을 재시도해
기존 URL을 반환하는지도 검증한다. 전체
단축 URL, 코드 해시, HMAC 지문, JDBC URL이나 Secret 값은 복원 검증 기록에 남기지
않는다.

다음 작업은 일상적인 되돌리기나 정리 명령으로 사용하지 않는다.
...
부분 초기화 복구다. 그 외에는 이름이 같더라도 기존 PVC로 취급해 백업·수동 계정 복구를 진행하거나
복원 담당자에게 보고한다.
...
- 만료·폐기 후 `410 LINK_EXPIRED`·`410 LINK_REVOKED`를 유지할 기간과
  삭제 후 `404` 전환 허용 여부
- 링크와 생성 예약을 함께 삭제할지, 자동 삭제 표시와 요청 비교 정보를 남길지 결정
