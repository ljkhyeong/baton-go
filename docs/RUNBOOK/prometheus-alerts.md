...
5. Prometheus의 기존 `alerting.alertmanagers`가 운영 Alertmanager를 가리키는지 확인한다.
   ...
   [Discord](external-api-integrations.md#discord로-go-장애-알림) 또는
   [Slack](external-api-integrations.md#slack으로-go-장애-알림)을 사용한다면 해당 receiver 예시를 병합한다.
...
| `BatonGoReadinessFailed` | 최근 2분 준비 상태 요청 12건 이상·503 비율 50% 초과가 2분 지속 | `critical` |
...
- 5xx 계산의 분자·분모에서 `/actuator...`, `/livez`, `/readyz`를 제외하고, 오류율 분모에서도
  ...
  HTTP 경보는 트래픽이 없거나 수집이 끊기면
  발생하지 않으므로 위의 수집 실패·대상 누락 경보를 함께 적용한다. 대상이 사라진 뒤 누락
  경보까지는 시계열의 stale 처리 시간과 2분 지속 시간이 걸린다.
- 준비 상태 실패는 Pod별 `/readyz` GET 요청을 사용한다. 최근 2분 요청이 12건 이상이고 503 비율이
  50%를 넘는 상태가 2분간 지속될 때 발생한다. 일시적인 실패와 정상 Pod를 분리하며, DB 또는
  활성화한 분산 요청 제한 Redis가 복구되어 정상 응답 비율이 회복되면 해제된다. Pod `NotReady`와
  DB·Redis 자체 장애 원인은 플랫폼 경보로도 확인한다.
