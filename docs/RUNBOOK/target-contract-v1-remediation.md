...
1. DB 백업과 현재 `BATON_GO_LINK_CODE_SECRET` 비밀값 관리자 버전을 하나의 복구 단위로
   보존하고 검증용 생성 의도 재생을 확인한다.
2. 정확한 대상 검증이 포함된 실행 파일을 먼저 배포한다. 정리가 끝날 때까지 그 이전 실행 파일로
   롤백하지 않는다.
3. 구버전 기록자, 직접 SQL 적재와 스키마 변경을 중지한다.
...
2. 기존 생성 의도를 재사용하지 않고 새 정규 UUID 생성 의도를 저장한다.
...
`HOLD`를 임의로 건너뛰고 관문을 완료 처리하면 안 된다.
