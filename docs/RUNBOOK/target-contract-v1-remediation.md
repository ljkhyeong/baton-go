...
이 운영 절차는 PRD-0003 이전에 저장된 링크를 점검하고 대상 규칙을 위반한 링크를
폐기하기 위한 절차다. API 구현이나 테스트 DB 정리는 운영 DB 점검 완료 기록을
대신하지 않는다.
...
2. `TrustedTargetPolicy` 검증을 포함한 이미지를 먼저 배포한다. 폐기가 끝날 때까지 이전 이미지로
   롤백하지 않는다.
3. 이전 버전의 링크 저장 프로세스, 직접 SQL 적재와 스키마 변경을 중지한다.
4. 공개 외부 주소에서 `/api/v1` 관리 경로를 인증 없이 요청한다. BATON GO의
   `WWW-Authenticate: Bearer realm="baton-go-management"` 또는 GO
...
환경 변수로 내보내지 않은 셸 변수로 읽어 curl 인자가 아닌 표준 입력 헤더로 전달한다. 기존 작업과
...
조회한다. 각 항목에는 저장된 대상 문자열이나 코드가 없으며 다음 정보만 사용한다.
...
승인 목록에는 `linkId`, 점검 시점의 `version`, 조치와 승인자만 기록한다. 조치는
다음 네 가지로 제한한다.

- `KEEP`: 대상 규칙을 충족하는 링크
- `REVOKE_ONLY`: 담당 서비스가 없거나 재발급이 불필요한 규칙 위반 링크
- `REISSUE_THEN_REVOKE`: 원본 담당 서비스가 정상 대체 링크를 먼저 만들 수 있는 행
- `HOLD`: 담당 서비스, 대상 매핑 또는 저장값이 올바른지 아직 확인하지 못한 행

대상 경로, 저장된 열거형 문자열, 코드 해시, 단축 URL, 멱등성 해시와 이 값들의 다이제스트를
승인 목록, 로그, 티켓 또는 CI 산출물에 추가하지 않는다. `creationRequestState=MISSING`은 자동 복구하지
않고 원본 담당 서비스가 확인할 때까지 `HOLD`로 둔다.

## 3. 재발급

`REISSUE_THEN_REVOKE`는 원본 데이터를 관리하는 서비스가 수행한다.

1. BATON/ROUND의 원본 매핑을 먼저 커밋한다.
...
점검 뒤 행이 변경되지 않았는지 확인하도록 승인 목록의 `version`을 보낸다.
...
- 점검 뒤 변경된 미폐기 행은 `409 REMEDIATION_STALE`이며 다시 점검·승인한다.
...
허용되지 않은 대상에 자격 증명이 포함됐을 가능성이 있으면 이 절차의 성공과 별개로 자격 증명을
