...
CHANNELS = {"slack": 9093, "discord": 9094}
...
def exercise(output):
    ...
    class Receiver(BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            self.end_headers()
            self.wfile.write(b'{"id":"fixture"}' if channel == "discord" else b"ok")
    ...
    try:
        for channel, port in CHANNELS.items():
        ...
        report = {
            ...
            "checks": ["발생·해제 알림", "429 이후 다음 주기 전송", "Discord 저장 확인 옵션", "리다이렉트 미전송", "다른 서비스 제외", "비공개 값 제외"],
...
def run(output):
    ...
    for channel in CHANNELS:
        ...
        example_url = (root / f"deploy/prometheus/{channel}-webhook-url.example").read_text().strip()
        endpoint = urlunsplit(("http", "127.0.0.1:18080", "/" + channel, urlsplit(example_url).query, ""))
        (output / f"{channel}-webhook-url").write_text(endpoint + "\n")
